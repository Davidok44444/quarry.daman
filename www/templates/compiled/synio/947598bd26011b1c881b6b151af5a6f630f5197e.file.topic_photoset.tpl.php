<?php /* Smarty version Smarty-3.1.8, created on 2012-09-11 15:30:07
         compiled from "/home/u224958/quarry.ru/www//templates/skin/synio/topic_photoset.tpl" */ ?>
<?php /*%%SmartyHeaderCode:1711313890504f20bfd6d4f2-62092874%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '947598bd26011b1c881b6b151af5a6f630f5197e' => 
    array (
      0 => '/home/u224958/quarry.ru/www//templates/skin/synio/topic_photoset.tpl',
      1 => 1345531563,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '1711313890504f20bfd6d4f2-62092874',
  'function' => 
  array (
  ),
  'variables' => 
  array (
    'oTopic' => 0,
    'oMainPhoto' => 0,
    'aLang' => 0,
    'bTopicList' => 0,
    'iPhotosCount' => 0,
    'oConfig' => 0,
    'aPhotos' => 0,
    'oPhoto' => 0,
    'iLastPhotoId' => 0,
  ),
  'has_nocache_code' => false,
  'version' => 'Smarty-3.1.8',
  'unifunc' => 'content_504f20bff1a280_82876498',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_504f20bff1a280_82876498')) {function content_504f20bff1a280_82876498($_smarty_tpl) {?><?php if (!is_callable('smarty_function_hook')) include '/home/u224958/quarry.ru/www//engine/modules/viewer/plugs/function.hook.php';
if (!is_callable('smarty_modifier_ls_lang')) include '/home/u224958/quarry.ru/www//engine/modules/viewer/plugs/modifier.ls_lang.php';
if (!is_callable('smarty_function_cfg')) include '/home/u224958/quarry.ru/www//engine/modules/viewer/plugs/function.cfg.php';
if (!is_callable('smarty_modifier_declension')) include '/home/u224958/quarry.ru/www//engine/modules/viewer/plugs/modifier.declension.php';
?><?php echo $_smarty_tpl->getSubTemplate ('topic_part_header.tpl', $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, null, null, array(), 0);?>



<script type="text/javascript">
	jQuery(window).load(function($) {
		ls.photoset.showMainPhoto(<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getId();?>
);
	});
</script>


<?php $_smarty_tpl->tpl_vars['oMainPhoto'] = new Smarty_variable($_smarty_tpl->tpl_vars['oTopic']->value->getPhotosetMainPhoto(), null, 0);?>
<?php if ($_smarty_tpl->tpl_vars['oMainPhoto']->value){?>
<div class="topic-photo-preview" id="photoset-main-preview-<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getId();?>
" onclick="window.location='<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getUrl();?>
#photoset'">
	<div class="topic-photo-count" id="photoset-photo-count-<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getId();?>
"><?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getPhotosetCount();?>
 <?php echo $_smarty_tpl->tpl_vars['aLang']->value['topic_photoset_photos'];?>
</div>
	
	<?php if ($_smarty_tpl->tpl_vars['oMainPhoto']->value->getDescription()){?>
		<div class="topic-photo-desc" id="photoset-photo-desc-<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getId();?>
"><?php echo $_smarty_tpl->tpl_vars['oMainPhoto']->value->getDescription();?>
</div>
	<?php }?>
	
	<img src="<?php echo $_smarty_tpl->tpl_vars['oMainPhoto']->value->getWebPath(500);?>
" alt="image" id="photoset-main-image-<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getId();?>
" />
</div>
<?php }?>

<?php $_smarty_tpl->tpl_vars['iPhotosCount'] = new Smarty_variable($_smarty_tpl->tpl_vars['oTopic']->value->getPhotosetCount(), null, 0);?>
<div class="topic-content text">
	<?php echo smarty_function_hook(array('run'=>'topic_content_begin','topic'=>$_smarty_tpl->tpl_vars['oTopic']->value,'bTopicList'=>$_smarty_tpl->tpl_vars['bTopicList']->value),$_smarty_tpl);?>

	
	<?php if ($_smarty_tpl->tpl_vars['bTopicList']->value){?>
		<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getTextShort();?>

		<?php if ($_smarty_tpl->tpl_vars['oTopic']->value->getTextShort()!=$_smarty_tpl->tpl_vars['oTopic']->value->getText()){?>
			<br />
			<a href="<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getUrl();?>
#cut" title="<?php echo $_smarty_tpl->tpl_vars['aLang']->value['topic_read_more'];?>
">
				<?php if ($_smarty_tpl->tpl_vars['oTopic']->value->getCutText()){?>
					<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getCutText();?>

				<?php }else{ ?>
					<?php echo smarty_modifier_ls_lang($_smarty_tpl->tpl_vars['aLang']->value['topic_photoset_show_all'],"COUNT%%".($_smarty_tpl->tpl_vars['iPhotosCount']->value));?>
 &rarr;
				<?php }?>                           
			</a>
		<?php }?>
	<?php }else{ ?>
		<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getText();?>

	<?php }?>
	
	<?php echo smarty_function_hook(array('run'=>'topic_content_end','topic'=>$_smarty_tpl->tpl_vars['oTopic']->value,'bTopicList'=>$_smarty_tpl->tpl_vars['bTopicList']->value),$_smarty_tpl);?>

</div> 


<?php if (!$_smarty_tpl->tpl_vars['bTopicList']->value){?>
	<script type="text/javascript" src="<?php echo smarty_function_cfg(array('name'=>'path.root.engine_lib'),$_smarty_tpl);?>
/external/prettyPhoto/js/prettyPhoto.js"></script>	
	<link rel='stylesheet' type='text/css' href="<?php echo smarty_function_cfg(array('name'=>'path.root.engine_lib'),$_smarty_tpl);?>
/external/prettyPhoto/css/prettyPhoto.css" />
	<script type="text/javascript">
		jQuery(document).ready(function($) {	
			$('.photoset-image').prettyPhoto({
				social_tools:'',
				show_title: false,
				slideshow:false,
				deeplinking: false
			});
		});
	</script>
	
	
	<div class="topic-photo-images">
		<h2><?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getPhotosetCount();?>
 <?php echo smarty_modifier_declension($_smarty_tpl->tpl_vars['oTopic']->value->getPhotosetCount(),$_smarty_tpl->tpl_vars['aLang']->value['topic_photoset_count_images']);?>
</h2>
		<a name="photoset"></a>
		
		<ul id="topic-photo-images">
			<?php $_smarty_tpl->tpl_vars['aPhotos'] = new Smarty_variable($_smarty_tpl->tpl_vars['oTopic']->value->getPhotosetPhotos(0,$_smarty_tpl->tpl_vars['oConfig']->value->get('module.topic.photoset.per_page')), null, 0);?>
			<?php if (count($_smarty_tpl->tpl_vars['aPhotos']->value)){?>                                
				<?php  $_smarty_tpl->tpl_vars['oPhoto'] = new Smarty_Variable; $_smarty_tpl->tpl_vars['oPhoto']->_loop = false;
 $_from = $_smarty_tpl->tpl_vars['aPhotos']->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
foreach ($_from as $_smarty_tpl->tpl_vars['oPhoto']->key => $_smarty_tpl->tpl_vars['oPhoto']->value){
$_smarty_tpl->tpl_vars['oPhoto']->_loop = true;
?>
					<li><a class="photoset-image" href="<?php echo $_smarty_tpl->tpl_vars['oPhoto']->value->getWebPath(1000);?>
" rel="[photoset]"  title="<?php echo $_smarty_tpl->tpl_vars['oPhoto']->value->getDescription();?>
"><img src="<?php echo $_smarty_tpl->tpl_vars['oPhoto']->value->getWebPath('50crop');?>
" alt="<?php echo $_smarty_tpl->tpl_vars['oPhoto']->value->getDescription();?>
" /></a></li>                                    
					<?php $_smarty_tpl->tpl_vars['iLastPhotoId'] = new Smarty_variable($_smarty_tpl->tpl_vars['oPhoto']->value->getId(), null, 0);?>
				<?php } ?>
			<?php }?>
			<script type="text/javascript">
				ls.photoset.idLast='<?php echo $_smarty_tpl->tpl_vars['iLastPhotoId']->value;?>
';
			</script>
		</ul>
		
		<?php if (count($_smarty_tpl->tpl_vars['aPhotos']->value)<$_smarty_tpl->tpl_vars['oTopic']->value->getPhotosetCount()){?>
			<a href="javascript:ls.photoset.getMore(<?php echo $_smarty_tpl->tpl_vars['oTopic']->value->getId();?>
)" id="topic-photo-more" class="topic-photo-more"><?php echo $_smarty_tpl->tpl_vars['aLang']->value['topic_photoset_show_more'];?>
 &darr;</a>
		<?php }?>
	</div>
<?php }?>

 
<?php echo $_smarty_tpl->getSubTemplate ('topic_part_footer.tpl', $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, null, null, array(), 0);?>
<?php }} ?>