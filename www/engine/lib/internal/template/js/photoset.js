...
ls.photoset =( function ($) {
	...
	this.addPhoto = function(response) {
        ...
        this.addPhoto2 = function(response) {
		$('#photoset_photo_empty').remove();
		if (!response.bStateError) {
			template = '<li id="photo_'+response.id+'"><img src="'+response.file+'" alt="image" style="max-width:100px"/>'
						+'<textarea onBlur="ls.photoset.setPreviewDescription('+response.id+', this.value)" name="'+response.id+'" onclick="edit_des()"></textarea><br /></li>';
			$('#swfu_images').append(template);
			ls.msg.notice(response.sMsgTitle,response.sMsg);
		} else {
			ls.msg.error(response.sMsgTitle,response.sMsg);
	...
	{
            var files = $('#photoset-upload-form')[0][0].files;
        ...
        for (var i = 0, f; f = files[i]; i++) {
            ...
            readers[i].onload = function(event) {
                ...
                data.id = 1;
                data.file = url;
                ls.photoset.addPhotoImage(data);
            }
                readers[i].readAsDataURL(f);
        }
		...
		ls.ajaxSubmit(aRouter['photoset']+'uploadimage/',$('#photoset-upload-form'),function(data){			
                        if (data.bStateError) {
				$('#photoset_photo_empty').remove();
				ls.msg.error(data.sMsgTitle,data.sMsg);
			} else {
